...
const OTHER_TARGETS_TOOLS = [
  'workspace', 'explorer', 'vms', 'terminal', 'settings', 'chat', 'gwz', 'wyred.census',
];
...
describe('the gyld target plugin list', () => {
  ...
  it('advertises every Gyld tool and nothing else', async () => {
    await expect.poll(() => Object.keys(tools()).sort()).toEqual(GYLD_TOOLS);
