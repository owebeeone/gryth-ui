...
It replaced a `decision-streams-v5` copy, which replaced a `v3` one, which
replaced a `v2` one. What the v5 run added is what the stream, decide and diff
...
(`gyld/examples/README.md`, "Which streams exist, and how a host knows"). The
BASE stream is byte identical to v5 apart from the build stamp, because the
base declaration carries no block; every overlay stream's text fingerprint,
snapshot digest and lens files changed, because the block is part of the module
text the capture reads. `fork-a` was regenerated by the host as well, so its
note is the generated `fork of stream-a` rather than the hand-written line v5
carried. The stream record still emits no `follows` and no `imports` field:
those two are read off the emitted `chain`, which is what the capture host
walks `follows` to build.
